defmodule Plaid.UtilsTest do
  ...
  describe "handle_response/2" do
    ...
    test "return Plaid error" do
      ...
      assert {:error, %Plaid.Error{}} = Plaid.Utils.handle_resp(resp, :any)
    end

    test "return HTTPoison error" do
      resp = {:error, %HTTPoison.Error{}}
      assert {:error, %HTTPoison.Error{}} = Plaid.Utils.handle_resp(resp, :any)
    end
    ...
    test "map categories response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :categories)
    ...
    test "map income response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :income)
    ...
    test "map institutions response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :institutions)
    ...
    test "map institution response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :institution)
    ...
    test "map transactions response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :transactions)
    ...
    test "map accounts response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :accounts)
    ...
    test "map auth response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :auth)
    ...
    test "map identity response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :identity)
    ...
    test "map item response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :item)
    ...
    test "parse new_access_token from item response" do
      ...
      assert {:ok, %{new_access_token: "access-sandbox-7c69d345-fd46"}} =
               Plaid.Utils.handle_resp(resp, :item)
    end
    ...
    test "parse request_id from item response" do
      ...
      assert {:ok, %{request_id: "45QSn"}} = Plaid.Utils.handle_resp(resp, :item)
    end
    ...
    test "map holdings from investment response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :"investments/holdings")
    ...
    test "map transactions from investment response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :"investments/transactions")
    ...
    test "map link response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :link)
    ...
    test "map webhook_verification_key response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :webhook_verification_key)
    ...
    test "map single payment response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :"payment_initiation/payment")
    ...
    test "map single recipient response to data structure" do
      ...
      assert {:ok, ds} = Plaid.Utils.handle_resp(resp, :"payment_initiation/recipient")
